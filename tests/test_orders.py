...
class TestCreateOrder:
    ...
    def test_create_order_with_authorization(self, registration_user_fixture):
        register_user = next(registration_user_fixture)
        user = register_user(USER_DATA)
        api = APIClient()
        response_auth = api.auth_user(USER_DATA['email'], USER_DATA['password'])
        response_create = api.create_order(["61c0c5a71d1f82001bdaaa70"])
    ...
    def test_create_order_without_authorization(self):
        api = APIClient()
        response_create = api.create_order(["61c0c5a71d1f82001bdaaa71"])
        assert response_create.status_code == 200
    ...
    def test_create_order_with_invalid_ingredients(self):
        api = APIClient()
        response_create = api.create_order(["incorrect"])
