...
class TestProfileUpdate:
    ...
    def test_update_user_with_authorization(self, registration_user_fixture, updated_name, updated_password):
    ...
    def test_update_user_with_authorization_with_mail_used(self, registration_user_fixture):
        register_user = next(registration_user_fixture)
        user = register_user(USER_DATA)
        api = APIClient()
        response_auth = api.auth_user(USER_DATA['email'], USER_DATA['password'])
